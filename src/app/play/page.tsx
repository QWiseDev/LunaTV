  ...
  useEffect(() => {
    ...
    const initAll = async () => {
      ...
      if (
        ...
        // 进入优选阶段，但不阻塞播放：先用可播放的源起播
        setLoadingStage('preferring');
        setLoadingMessage('⚡ 正在优选最佳播放源...');

        // 1) 先行起播：优先使用已指定的源，否则使用首个搜索结果
        const earlyDetail =
          currentSource && currentId
            ? sourcesInfo.find(
        ...
        // 提交起播所需状态
        setNeedPrefer(false);
        setCurrentSource(earlyDetail.source);
        setCurrentId(earlyDetail.id);
        setVideoYear(earlyDetail.year);
        setVideoTitle(earlyDetail.title || videoTitleRef.current);
        setVideoCover(earlyDetail.poster);
        // 优先保留URL参数中的豆瓣ID，如果URL中没有则使用详情数据中的
        setVideoDoubanId(
          videoDoubanIdRef.current || earlyDetail.douban_id || 0
        );
        setDetail(earlyDetail);
        if (currentEpisodeIndex >= earlyDetail.episodes.length) {
          setCurrentEpisodeIndex(0);
        }

        // 同步URL（不刷新）
        const earlyUrl = new URL(window.location.href);
        earlyUrl.searchParams.set('source', earlyDetail.source);
        earlyUrl.searchParams.set('id', earlyDetail.id);
        earlyUrl.searchParams.set('year', earlyDetail.year);
        earlyUrl.searchParams.set('title', earlyDetail.title);
        earlyUrl.searchParams.delete('prefer');
        window.history.replaceState({}, '', earlyUrl.toString());

        // 显示就绪并关闭加载态
        setLoadingStage('ready');
        setLoadingMessage('✨ 已开始播放，后台优选中...');
        setTimeout(() => setLoading(false), 400);

        // 2) 后台进行优选测速；如最佳源不同，自动无缝换源
        preferBestSource(sourcesInfo)
          .then((best) => {
            // 若后台优选结果与起播源不同，则平滑切换
            if (
              best.source !== earlyDetail.source ||
              best.id !== earlyDetail.id
            ) {
              // 使用已有的防抖/保进度换源逻辑
              handleSourceChange(
                best.source,
                best.id,
                best.title || videoTitleRef.current
              );
            }
          })
          .catch((e) => {
            console.warn('后台优选失败，保持当前播放源:', e);
          });
